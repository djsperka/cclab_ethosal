classdef eyetracker < handle
    ...
    methods
        function obj = eyetracker(mode, name, window)
            ...
            try
                p = inputParser;
                p.addRequired('Mode', @(x) isscalar(x) && (x == 0 || x == 1));
            ...
            if mode == 1
            elseif mode == 0
                % standard setup for cclab
                obj.eyelink_setup();
            else
                errID = 'eyetracker:BadMode';
                msg = 'Mode must be 0 (use tracker) or 1 (dummy).';
        ...
        function [] = command(obj, varargin)
            ...
            switch (obj.Mode)
                ...
                otherwise
                    errID = 'eyetracker:command';
                    msg = 'Mode must be 0,1, or 2.';
                    baseException = MException(errID,msg);
                    throw(baseException);
            end
        end
    ...
    methods (Access = private)
        function eyelink_setup(obj)
            [result, ~] = EyelinkInit(obj.Mode)
            if ~result
                errID = 'eyetracker:eyelink_setup';
                msg = sprintf('EyelinkInit failed to connect (mode %d).', obj.Mode);
                baseException = MException(errID,msg);
                throw(baseException);
            end
