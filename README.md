...
Input trials are generated by the script `generateEthBlocksImgV2`. [A detailed description of how the current input files are generated can be found here](ethosal_blocks.md). Older versions were generated by the other similarly-named scipts (they're no longer used and probably will not run). 
...
![ethosal dialog](ethosalDlgV2Goals.png "ethosal main dialog")

The items in the dialog and their meanings:

- **Subject ID** - An identifier for the subject. Will be incorporated into output file.
- **Test Type** - **Always choose "Flip"** There have been different variations of the experiment in the past, and the older ones may not work any more.
- **Location** - Select *Booth(subj)* to run a subject with eye tracking in the EEG booth. Select *Booth(Test) to do the same without eye tracking. Select *Desk* to run on Dan's desk (won't work in booth).
- **Scr Dist (mm), Scr W,H (mm)** - Screen distance (eye to center of screen), as well as width and height, in mm. (Do not change these values)

The checkboxes in the *Options* section are for testing only!
...
- **Use booth kbd** -for testing one's self in the booth. Matlab will be told to listen to the booth keyboard for experimenter's input (e.g. \<space\> for pause, etc.)
