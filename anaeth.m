function [ratesNone, ratesIn, ratesOut] = anaeth(attendLeft, attendRight, attendNone)
...
%     z=load(matfile);    
    ...
    ratesNone = ethrates(attendNone, logsNone);
    ...
    maskLeft = vertcat(true(height(attendLeft), 1), false(height(attendRight), 1));
    maskRight = vertcat(false(height(attendLeft), 1), true(height(attendRight), 1));
...
%ethlogs(A) Generate logical masks for analyzing data.
    ...
    % etc. In etholog, the 'StimPairType' refers to the left-right
    ...
    % These indicate trials where the response was Left, Right, or None.
